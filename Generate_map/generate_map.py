...
def generate_height_map(fname):
    
    scene_id = int(fname[2:5])
    
    path_mesh = load_dir_sem + fname + '/' + fname[6:17] + '.semantic.glb'
...
def generate_texture_map(fname):
    
    scene_id = int(fname[2:5])
    
    path_mesh_3d = load_dir_3d + fname + '/' + fname[6:17] + '.obj'
    path_texture2 = load_dir_3d + fname + '/'
    ...
    mesh = o3d.io.read_triangle_mesh(path_mesh_3d)
    mesh.compute_vertex_normals()
    ...
    cv2.imwrite(path_save + fname[2:5] + '.png', np.swapaxes(img_o,0,1))

def generate_semantic_map(fname):
    
    color_type = -1  #coloring strategy for semantic label, -1: raw color in 3D model 0: color by original semantic index 1: #color by nyuid 2: #color by nyu40id
    
    scene_id = int(fname[2:5])
    
    path_mesh_3d = load_dir_3d + fname + '/' + fname[6:17] + '.obj'
    path_mesh_sem = load_dir_sem + fname + '/' + fname[6:17] + '.semantic.glb'
    ...
    if color_type != -1:
        id2sem = np.genfromtxt(load_dir_sem + fname + '/' + fname[6:17] + ".semantic.txt", delimiter=',', skip_header=1, dtype=None, encoding='utf-8')
    ...
    make_texturefiles_from_mesh(path_mesh_sem, path_texture)
...
if __name__ == "__main__":
    ...
    load_dir_sem = params['path_habitat_sem']
    load_dir_3d = params['path_habitat']
    
    size_px = np.array([params['size_px'],params['size_px']])
    size_m = np.array([params['size_m'],params['size_m']])
    
    fname = os.listdir(load_dir_sem)
    
    for i in range(len(fname)):
        
        if params['flag_binary']==1:
            generate_binary_map(fname[i])
        if params['flag_height']==1:
            generate_height_map(fname[i])
        if params['flag_semantic']==1:
            generate_semantic_map(fname[i])
        if params['flag_texture']==1:
            generate_texture_map(fname[i])
